...
module axil_simple_tb;
  ...
  initial begin
    ...
    @(posedge clk);
    new_tx = 1'b0;
    wait (dut.u_master.wstate != 0);   // wait for FSM to actually go busy
    ...
    // 2. READ: addr=3, expect AAAABBBB.
    raddr  = 32'd3;
    wr     = 1'b0;
    new_tx = 1'b1;
    @(posedge clk);
    new_tx = 1'b0;
    wait (dut.u_master.rstate != 0);   // wait for FSM to actually go busy
    ...
    // 3. BACK-TO-BACK WRITE then READ: addr=7.
    waddr  = 32'd7;
    din    = 32'hCAFEF00D;
    wr     = 1'b1;
    new_tx = 1'b1;
    ...
    // fire read immediately, no extra settle time
    raddr  = 32'd7;
    wr     = 1'b0;
    new_tx = 1'b1;
    @(posedge clk);
    ...
    wait (dut.u_master.rstate == 0 && dut.u_master.rd_count == 0);
    #20;
    $display("[%0t] W+R addr=7 -> dout=%h resp=%b (expected CAFEF00D)", $time, dout, resp);
